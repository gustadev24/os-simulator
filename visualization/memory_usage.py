    ...
    def generate(self, loader: MetricsLoader) -> None:
        ...
        if page_faults:
            ...
            ax2.step(
                ...
                fault_totals,
                where="post",
                color=self.CHART_COLORS["quaternary"],
                linewidth=self.STYLE["line_width"],
            )
            ax2.fill_between(
                fault_ticks,
            ...
            self.style_axis(
                ax2,
                xlabel="Tiempo (ticks)",
                ylabel="Fallos de Página Totales",
...
class FrameAllocationGenerator(BaseGenerator):
    ...
    def generate(self, loader: MetricsLoader) -> None:
        ...
        colors = [self.get_process_color(label) for label in labels[:-1]] + [
            self.CHART_COLORS["free"]
        ]
        ...
        ax1.set_title(
            "Distribución de Frames de Memoria",
            fontsize=self.FONT_SIZES["title"],
            fontweight="bold",
        )
        ...
        self.style_axis(
            ax2, ylabel="Frames Asignados", title="Frames por Proceso", grid_axis="y"
        )
