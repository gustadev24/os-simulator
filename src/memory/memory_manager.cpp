...
void MemoryManager::unregister_process(int pid) {
    ...
    // Remove queued tasks for this process
    fault_queue.erase(
        std::remove_if(fault_queue.begin(), fault_queue.end(),
                       [pid](const PageLoadTask &task) {
                           return task.process && task.process->pid == pid;
                       }),
        fault_queue.end());
...
void MemoryManager::start_next_task_if_possible(int current_time) {
    if (active_task || fault_queue.empty()) return;
...
std::shared_ptr<Process> MemoryManager::complete_active_task(int completion_time) {
    if (!active_task || !active_task->process) return nullptr;
