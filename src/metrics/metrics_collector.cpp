...
bool MetricsCollector::enable_file_output(const std::string &path) {
  ...
  try {
    file_out = std::make_unique<std::ofstream>(path, std::ios::out | std::ios::app);
    if (!file_out->is_open()) {
      file_out.reset();
...
void MetricsCollector::flush_tick(int tick)
{
    ...
    if (it == tick_buffer.end()) return;

    const TickData &data = it->second;
